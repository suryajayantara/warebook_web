              ...
              <div class="card-body">
                <form enctype="multipart/form-data" action="{{ route('studies.update', $study_data->id) }}" method="post">
                  ...
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <select name="departement_id" id="id_faculty" class="form-control @error('departement_id') is-invalid @enderror" aria-label="Default select example" required>
                            ...
                            <option value="{{$study_data->departement_id}}" selected>{{$study_data->departements->departement_name}}</option>
                            @foreach ($departement_data as $departement)
                                @if ($study_data->departement_id != $departement->id)
                  ...
                  <div class="row mb-3">
                    <label class="col-sm-2 col-form-label" >Nama</label>
                    <div class="col-sm-10">
                        <input value="{{$study_data->studies_name}}" name="studies_name" type="text" class="form-control" placeholder="Nama Jurusan" />
                        @error('studies_name')
                            <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                            </span>
